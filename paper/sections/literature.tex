...
\begin{enumerate}
  ...
  \item Vendor-native telemetry (AMD \gls{SMU} / PM tables) is promising but
...
\gls{RAPL} and AMD-native telemetry (\gls{SMU} / PM tables) on AMD \gls{CPU}s,
closing a documented gap in the literature by:

\begin{itemize}
  \item instrumenting both sources simultaneously,
  \item using a robust resampling and overflow-aware pipeline,
  \item applying both signal and agreement analyses (correlation,xi
    cross-correlation, Wilcoxon signed-rank, Bland–Altman, \gls{RMSE}/
    \gls{MAE}/\gls{MAPE}) to quantify differences.
\end{itemize}
