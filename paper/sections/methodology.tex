...
attempts to poll faster \parencite{ryzen_managment_linux}. While this project
...
In the implementation we keep internal counters in micro-joules; hence,
...
exposes the accumulated micro-joules via \gls{EMA}'s API.
...
\begin{itemize}
  ...
  \item For \gls{RAPL}/\gls{MSR} measurements \gls{EMA}'s overflow utilities
    are applied to handle counter wraparound; for \gls{SMU} readings wrap
    behavior is handled according to the \gls{SMU} field semantics.
\end{itemize}

On finalization the plugin releases the \texttt{libsmu} context and
frees all per-device structures registered with \gls{EMA}.

This concise design allows simultaneous collection of \gls{SMU} (Ryzen) and
