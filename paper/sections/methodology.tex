...
\emph{core} domains follow similar temporal trends and exhibit a high degree
of dynamic correlation.
\end{minipage}
\end{equation*}

To evaluate the above the study focuses on complementary analyses:
\begin{itemize}
  \item Pearson correlation and cross-correlation to quantify overall dynamic
        similarity and detect potential time lags,
  \item visual inspection of synchronized traces to compare short-term
        fluctuations and long-term trends,
  \item Bland–Altman plots and basic error metrics (e.g., \gls{RMSE}, \gls{MAE})
        to describe the spread of differences and practical agreement.
\end{itemize}

This combination highlights temporal coherence and captures the general
agreement of the two data sources without requiring an external ground-truth
measurement.
...
implementation used for polling-rate validation is available in the
supplementary repository \parencite{yahdzhyiev2025repo}.
